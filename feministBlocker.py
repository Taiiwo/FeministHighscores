...
def scanUser(id):# scans a user id
	# grab other tweets by the user
	userTweets = []
	tempId = None
	if debugging == True: print "[ ] Scanning tweets by user. This may take some time"
	while 1:
		temp = twApi.GetUserTimeline(user_id = id, count = 200, max_id = tempId)
		tempId = temp[-1:][0].id
		userTweets += temp
		if len(temp) < 200:
			break
...
if __name__ == '__main__':
	...
	for index, tense in enumerate(tenses):
		...
		for term in tense:
			...
			hashtagTweets = twApi.GetSearch(term=term, count=100)
			...
			for tweet in hashtagTweets:
				...
				if tweet.user.name not in scannedUsers and tweet.text not in scannedTweets:
					...
					if index == 0 and score < 0 or index == 1 and score > 0:
						...
						# the tweet is probably retarded.
						# scan and score the user
						scanUser(tweet.user.id)
